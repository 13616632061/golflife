...
import com.glorystudent.golflife.adapter.LabelGridAdapter;
...
import com.glorystudent.golflife.entity.EventWithSignInfoEntity;
import com.glorystudent.golflife.entity.ReleasedRequestEntity;
import com.glorystudent.golflife.entity.SignDefinitionEntity;
...
public class EventReleasedActivity extends BaseActivity {
    ...
    private final static int SUCCESS = 0x123;//上传成功
    private final static int FAILURE = 0x321;//上传失败
    ...
    private boolean editFlag = false;//是否编辑赛事活动
    ...
    private List<SignDefinitionEntity.ListsigndefinitionBean> labelList;//赛事活动报名数据集合
    private LabelGridAdapter labelGridAdapter;//赛事活动报名列表适配器
    private String rootUrl;//根路径
    private PickerViewUtil pickerViewUtil;//时间滚动选择器
    private List<String> costTypeOptions;//费用类型下拉选择集合
    private List<String> guestTypeOptions;//是否允许下拉选择集合
    private List<String> passwordTypeOptions;//有无下拉选择集合
    private int teamId;
...
//        editEventValue();//编辑活动时调用
    ...
    private void initImageBanner() {
        ...
        ImageUtil.saveBitmapFile(bitmap, url);
        imageList.add(url);
    ...
    private void requestEventData() {
        ...
        OkGo.post(ConstantsURL.QueryEventActivityByID)
                .tag(this)
    ...
    private void getLabelData() {
        if (editFlag && listsigndefinition != null) {//编辑状态时直接呈现
            ...
            Log.i(TAG, "loadDatas: " + requestJson);
            OkGo.post(ConstantsURL.QuerySignDefinition)
                    .tag(this)
                    .params("request", requestJson)
                    .execute(new StringCallback() {
                        @Override
    ...
    private void editReleasedEvent() {
        ...
        OkGo.post(ConstantsURL.EditEventActivity)
                .tag(this)
                .params("request",requestJson)
                .execute(new StringCallback() {
                    @Override
    ...
    private void addReleasedEvent() {
        ...
        OkGo.post(ConstantsURL.AddEventActivity)
                .tag(this)
                .params("request",requestJson)
                .execute(new StringCallback() {
                    @Override
