...
public class OkGoRequest {
    ...
    public void getAliyunOSS(Context context) {
        okgocontext = context;
        String request = RequestAPI.getEmptyParameter(context);
        Log.i(TAG, "getAliyunOSS: " + request);
        OkGo.post(ConstantsURL.SelectAllByOSS)
                .tag(context)
                ...
                .execute(new StringCallback() {
                    ...
                    public void onSuccess(String s, Call call, Response response) {
                        if (onOkGoUtilListener != null) {
                            Log.i(TAG, "-----------------------------------------start-------------------------------");
                            Log.i(TAG, "URL:{"+ConstantsURL.BASE_URL + url+"}");
    ...
    public void upLoadFile(Context context, String filePath) {
        ...
        //上传路径
        String url = ConstantsURL.BASE_URL + "/api/APIUpLoadFile/UploadFile";
        Log.d("ceshi", "upLoadFile: --->测试");
        String request = RequestAPI.getFileUp(context, headPicMd5, headPicName, partmd5, bitmapSize, bitmapSize, partdata);
